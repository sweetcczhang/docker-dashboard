...
def discover_version(client, requested_version):
    ...
    elif harbor.API_MIN_VERSION > server_end_version:
        raise exceptions.UnsupportedVersion(
            ("Server version is too old. The client valid version range is "
             ...
             "range is '%(server_min)s' to '%(server_max)s'.") % {
                 'client_min': harbor.API_MIN_VERSION.get_string(),
                 'client_max': harbor.API_MAX_VERSION.get_string(),
                 'server_min': server_start_version.get_string(),
                 'server_max': server_end_version.get_string()})
    elif harbor.API_MAX_VERSION < server_start_version:
        raise exceptions.UnsupportedVersion(
            ("Server version is too new. The client valid version range is "
             ...
             "range is '%(server_min)s' to '%(server_max)s'.") % {
                 ...
                 'server_min': server_start_version.get_string(),
                 'server_max': server_end_version.get_string()})
    elif harbor.API_MAX_VERSION <= server_end_version:
        return harbor.API_MAX_VERSION
    elif server_end_version < harbor.API_MAX_VERSION:
